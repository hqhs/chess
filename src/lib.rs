...
impl Camera {
  fn new(aspect_ratio: f32) -> Self {
    let perspective =
      camera::Projection::InfinitePerspective { vfov: 45.0, near: 0.1 };
    ...
    let flying = camera::Flying { yaw, pitch, roll, translation, scale };

    // let view = Self::view_from_eye_and_point();
    // let view = Self::isometric_view();
    let speed = 0.1;
    let view = projection * flying.view();
    Self { view, flying, speed, aspect_ratio, projection, perspective }
  }
  ...
  fn isometric_view() -> Mat4 {
    camera::Isometric { looking_at: Vec3::ZERO, distance: 5.0 }.view()
  }
}
...
pub async fn run() -> anyhow::Result<()> {
  ...
  let egui_renderer =
    EguiRenderer::new(&iad.device, preferred_format, None, 1, &window);
  ...
  let debug_grid =
    Grid::new(preferred_format, &iad.device, &iad.queue, &grid_input);
  let mut game = Game {
    iad,
    background_color,
    egui_renderer,
  ...
  match event {
    ...
    Event::WindowEvent { event: WindowEvent::RedrawRequested, .. } => {
      let surface_texture = match surface.get_current_texture() {
        ...
        Err(e) => {
          log::error!(
            "failed to get surface texture to draw next frame: {}",
            e
          );
          return Ok(());
        }
      ...
      let mut frame = Frame { encoder, surface_texture, view };

      redraw(game, &mut frame);
      redraw_ui(&window, game, &mut frame);

      let Frame { encoder, surface_texture, .. } = frame;
...
fn maybe_move_camera(
  camera: &mut Camera,
  key: KeyEvent,
) -> EventResult {
  let KeyEvent { state, physical_key, .. } = key;
